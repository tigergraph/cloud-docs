...
function renderAuthorizations (operation, pad) {
  ...
  for (const requirement of operation.security || []) {
    for (const name of Object.keys(requirement)) {
      ...
      // The publisher only rewrites in-content links that carry Antora's xref
      // classes; without them this relative href is resolved against the site
      // root and loses the /docs base path.
      out.push(
        indent(pad + 4) +
