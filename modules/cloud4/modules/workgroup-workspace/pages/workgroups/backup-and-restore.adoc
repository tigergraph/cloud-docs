...
[IMPORTANT]
====
Backup functionality is currently available only for xref:cloud4:workgroup-workspace:workspaces/writeread_readonly.adoc#_read_write_rw_workspace[Read-Write (RW) workspaces].
...
Next, learn more about how to xref:cloud4:workgroup-workspace:workspaces/connect-via-api.adoc[] a workspace, or xref:cloud4:workgroup-workspace:workgroups/monitor-workspaces.adoc[].
