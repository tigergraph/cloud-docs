...
A: New November 2024 pricing applies to new workspaces provisioned on TigerGraph Cloud 4. The clusters provisioned in TigerGraph Cloud Classic will continue to be billed at the old pricing. If you want to take advantage of the new pricing to lower your cost, you can migrate your workload to TigerGraph Cloud 4. Note that only TigerGraph Cloud accounts with valid credit cards are eligible for the paid tier migration. Please submit a support ticket for migration assistance.
...
A: TigerGraph Cloud 4.0 offers different billing models and pricing options to accommodate various usage scenarios and customer needs. Here are the available billing models:
...
* Prepaid: The prepaid model allows you to purchase TigerGraph Credits (TCRs) in advance, which can be used to cover the costs of your TigerGraph Cloud usage. With prepaid credits, you have the advantage of predictable costs and the ability to allocate resources as needed within the prepaid budget.
...
=== *Q: If I add an workspace mid-month, when do I start paying for it?*
...
Root Disk: An EBS (Elastic Block Store) based root disk is attached to the TigerGraph workspace.
...
A: When you provision or restart a workspace, there will be a warm-up period for the workspace and the disk attached to the workspace.
...
* **Backups**: All backup copies are deleted.
...
A: Please refer to We will keep the latest backup for 15 days and then delete.
...
A: When you register your account, you will select a username and password. You can then log in anytime at link:http://portal.tgcloud.io/[portal.tgcloud.io]. You will also be given a URL using a subdomain name that you select.
