...
[IMPORTANT]
====
Backup functionality is currently available only for xref:cloud4:resource-manager:workspaces/writeread_readonly.adoc#_read_write_rw_workspace[Read-Write (RW) workspaces].
...
Next, learn more about how to xref:cloud4:resource-manager:connect_via_api.adoc[] a workspace, or xref:cloud4:resource-manager:monitor_workspaces.adoc[].
